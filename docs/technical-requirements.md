...
- **Derived-data caching (the single highest-leverage pattern)**: cache the *computed result*, not just inputs. The generated week (schedule + balance score + flags) is stored as one complete document. Every screen that needs "this week" reads from that one cached object — viewing Wednesday's load, checking the balance score, reviewing the event list are all *local filters on already-loaded data*, never separate queries. Recompute only when the user explicitly taps "Build my week" or "Redo" in the Plan tab — never silently in the background. Adding/editing an event does NOT trigger a rebuild; the event sits as "unscheduled" until the next explicit build (deleting an event still recomputes immediately, since stored balance score / heavy-day flags would otherwise misrepresent a week that no longer contains it).
